...
export default function Projects() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Projekte"
        subtitle="Ihre laufenden und abgeschlossenen Projekte"
        icon={FolderKanban}
      />
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Suche nach Projekt-Nr. oder Titel..."
            value={search}
          ...
          {selectedProject && (
            <div className="space-y-6 mt-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-slate-50 rounded-xl p-4">
                  <p className="text-xs text-slate-500 mb-1">Fortschritt</p>
                  <p className="text-xl font-bold text-slate-800">{selectedProject.progress || 0}%</p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
                  <p className="text-xs text-slate-500 mb-1">Startdatum</p>
                  <p className="font-medium text-slate-700">
                    {selectedProject.start_date 
                      ? format(new Date(selectedProject.start_date), 'dd. MMMM yyyy', { locale: de })
                      : '-'
                    }
                  </p>
                </div>
                <div className="bg-slate-50 rounded-xl p-4">
                  ...
                  <p className="font-medium text-slate-700">
                    {selectedProject.end_date 
                      ? format(new Date(selectedProject.end_date), 'dd. MMMM yyyy', { locale: de })
                      : '-'
                    }
                  </p>
                </div>
              ...
              {selectedProject.tasks?.length > 0 && (
                <div>
                  <h4 className="font-semibold text-slate-800 mb-3">Aufgaben</h4>
                    ...
                    {selectedProject.tasks.map((task, idx) => (
                      <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                        ...
                        <div className="flex-1">
                          <p className={`font-medium ${task.status === 'erledigt' ? 'text-slate-500 line-through' : 'text-slate-700'}`}>
                            {task.title}
                          </p>
                          {task.due_date && (
